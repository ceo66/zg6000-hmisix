...
function ApprovalDestroyRequest() {
			...
			render: (text, record) => (
				<Space size="middle">
					<Button
						type="primary"
						onClick={() => {
							currentExamId.current = record.finishWorkExamID;
							currentId.current = record.id;
							setExamParam({
								show: true,
								taskID: record.id, //taskID
								examID: record.finishWorkExamID, //examID
								onClose: () => {
									setExamParam({
										show: false,
										taskID: "", //taskID
	...
	useEffect(() => {
		setData(taskList.filter((item) => item.stageID === "ZG_WS_FINISH_EXAM"));
	}, [taskList]);
	useEffect(() => {
		setData(taskList.filter((item) => item.stageID === "ZG_WS_FINISH_EXAM"));
					...
					if (content.head.id === currentExamId.current) {
						...
						if (content.head.examStateID !== "ZG_ES_REJECT") {
							...
							if (!submittingApproval) {
								...
								stageFallback(currentId.current, clientUnique, serverTime);
							}
						}
					}
				}
	...
	useEffect(() => {
		...
		const extractedIds = data.map((item) => item.finishWorkExamID);
...
export default ApprovalDestroyRequest;
