...
export default class ITEdit extends PureComponent {
    ...
    componentDidMount() {
        constFn.postRequestAJAX(constVar.url.app.op.ITInfo, {
            ...
            if (result) {
                ...
                let rtStartTime = ITInfo.head?.rtStartTime;
                let rtEndTime = ITInfo.head?.rtEndTime;
                let workTime = null;
                ...
                this.refForm.current.setFieldsValue({
                    name: ITInfo.head?.name,
                    appNodeName: ITInfo.head?.appNodeName,
                    rtNumber: ITInfo.head?.rtNumber,
                    rtOperUserID: ITInfo.head?.rtOperUserID,
                    rtOperUserName: ITInfo.head?.rtOperUserName,
                    rtMonUserID: ITInfo.head?.rtMonUserID,
                    rtMonUserName: ITInfo.head?.rtMonUserName,
                    majorName: ITInfo.head?.majorName,
                    workTime: workTime
                });
            } else {
                    ...
                    <div style={{ height: "100%", display: "flex", overflow: "auto" }}>
                        ...
                        <div style={{ flex: 5, padding: "0px 6px", overflow: "auto" }}>
                            <Form
                                ...
                                layout="horizontal"
                                initialValues={{
                                    name: this.state.ITInfo.head?.name,
                                    appNodeName: this.state.ITInfo.head?.appNodeName,

                                    rtOperUserID: this.state.ITInfo.head?.rtOperUserID,
                                    rtOperUserName: this.state.ITInfo.head?.rtOperUserName,
                                    rtMonUserID: this.state.ITInfo.head?.rtMonUserID,
                                    rtMonUserName: this.state.ITInfo.head?.rtMonUserName,
                                    majorName: this.state.ITInfo.head?.majorName,
                                }}>
                        ...
                        <div style={{ flex: 6, padding: "0px 6px", overflow: "auto" }}>
                            <Table
                                ...
                                dataSource={this.state.ITInfo.items} />
                        </div>
                    </div>
                </Modal>
