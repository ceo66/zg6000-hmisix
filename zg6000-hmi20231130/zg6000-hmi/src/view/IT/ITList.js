...
import ITExecute from './Manager/ITExecute';
import ITEdit from './Manager/ITEdit';
import ITExamManager from './Manager/ITExamManager';

export default class ITList extends PureComponent {

    sysContext = null;
    refModalConfirm = React.createRef();
    mqttObj = {
        type: "op_param_task_it",
        topics: ["op_param_task/ZG_TT_IT/insert", "op_param_task/ZG_TT_IT/update", "op_param_task/delete"]
    }
    queryCriteria = {
        limit: 32,
        offset: 0,
            ...
            render: (text, record, index) => {
                return (
                    <Space>
                        ...
                        <span className={this.getTaskStateClassName(record.rtTaskStageID, record.rtTaskStateID)}>
                            {constFn.reNullStr(record.rtTaskStageName) + "【" + constFn.reNullStr(record.rtTaskStateName) + "】"}
                        </span>
                    </Space>
                )
            }
    ...
    getTaskList() {
        let condition = "a.taskTypeID='" + constVar.task.type.ZG_TT_IT + "'";
            ...
            onClick={() => {
                this.setState({
                    ITExamManagerParam: {
                        ...
                        show: true,
                        onClose: () => {
                            this.setState({ ITExamManagerParam: { ...this.state.ITExamManagerParam, ...{ show: false } } });
                        }
                    }
                });
            }}> 审批 </Button >;
    ...
    render() {
        ...
        return (
            <>
                ...
                {this.state.ITExamManagerParam.show ? <ITExamManager
                    ITId={this.state.ITExamManagerParam.ITId}
                    examId={this.state.ITExamManagerParam.examId}
                    onClose={this.state.ITExamManagerParam.onClose}></ITExamManager> : null}
                {this.state.showITEdit ? <ITEdit id={this.state.ITEditId} onClose={() => {
                    this.setState({ showITEdit: false });
                }}></ITEdit> : null}
                ...
                ></VerifyUser> : null}
                {this.state.showITExecute ? <ITExecute id={this.state.executeItId} onClose={() => {
                    this.setState({
                        showITExecute: false
                    });
                }}></ITExecute> : null}
