...
import VerifyUser from '../VerifyPower/VerifyUser';
    ...
    render() {
        return (
            <>
                ...
                <Modal
                    centered
                    title={<div style={{ textAlign: "center" }}>审批</div>}
                    ...
                    <ExamineExec taskID={this.taskID} examID={this.examID} onSuccess={() => {
...
class ExamineExec extends Component {

    constructor(props) {
        super(props);
        this.tastaskIDkId = props.taskID;//票ID
        this.examID = props.examID;//审批ID
        this.onSuccess = props.onSuccess;
        this.isFinish = false;
        this.sysContext = null;
        ...
        constFn.postRequestAJAX(constVar.url.app.sp.examInfo, {
            ...
            if (result) {
                this.setState({ examInfo: backJson.data }, () => {
                    let isExamFinish = true;
                    for (const iterator of this.state.examInfo.node) {
                        if (iterator.examStateID !== constVar.task.examState.ZG_ES_ACCEPT) {
                            isExamFinish = false;
                        }
                    }
                    if (isExamFinish) {//审批已经完成
                        //message.success("审批完成");
                        if (!this.isFinish) {
                            this.isFinish = true;
                            this.onSuccess && this.onSuccess();//直接回调成功，不再显示完成按钮
                        }
                    }
                });
            } else {
    ...
    getStepItems(nodes) {
        ...
        for (const iterator of nodes) {
            let itemObj = {};
            itemObj.title = iterator.name;
            itemObj.description = iterator.examResultName + (iterator.examInfo ? ("【" + iterator.examInfo + "】") : "");
            itemObj.status = this.getExamResult(iterator.examResultID);
                                                    ...
                                                    <>
                                                        ...
                                                        <div>
                                                            <Steps direction="vertical" size="small"
                                                                items={this.getStepItems(item.step)}>
                                                                {/* {
